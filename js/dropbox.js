...
dropbox.consumerKey = "0660jgq6erg4h63";
...
//Set to false to disable file md5etadata caching
dropbox.cache = true;
...
dropbox.createOauthRequest = function(param1,param2,callback) {
	...
	if (!param1.contentType) {
		param1.contentType = "application/x-www-form-urlencoded";
	}
...
dropbox.uploadFile = function(path, filename, content) {
	var message = dropbox.createOauthRequest({
		url: "http://api-content.dropbox.com/0/files/" + dropbox.accessType + "/" + path + "?file=" + filename,
		type: "text",
		method: "POST",
	...
	var url = "http://api-content.dropbox.com/0/files/" + dropbox.accessType + "/" + path + "?file=" + filename;
	...
	for (k in message.parameters) {
		body += '--' + boundary + '\r\n' + 'Content-Disposition: form-data; name="';
		body += message.parameters[k][0];
		body += '"\r\n\r\n';
		body += message.parameters[k][1];
		body += '\r\n';
	}
